...
/**
 * Caso de uso para añadir un asistente a un evento
 */
@Injectable()
export class AddAttendeeUseCase {
   ...
   * @param userId ID del usuario a añadir como asistente
   * @returns El evento actualizado con el nuevo asistente
   */
  async execute(eventId: string, userId: string): Promise<EventDto> {
    try {
      ...
      if (error instanceof EventAttendanceException) {
        throw new Error(`Error al añadir asistente: ${error.message}`);
      }
      throw error;
    }
  }
