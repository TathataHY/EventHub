...
export class CheckInCommand implements Command<CheckInParams, Attendance> {
  constructor(private readonly attendanceRepository: AttendanceRepository) {}

  async execute(params: CheckInParams): Promise<Attendance> {
    ...
    if (currentStatus === AttendanceStatus.CHECKED_IN) {
      throw new ValidationException('El usuario ya realizó el check-in');
    }
    
    if (currentStatus === AttendanceStatus.CHECKED_OUT) {
      throw new ValidationException('El usuario ya completó su asistencia (check-out)');
    }

    if (currentStatus === AttendanceStatus.CANCELLED) {
      throw new ValidationException('La asistencia fue cancelada y no puede realizarse check-in');
    }

    // Realizar el check-in
    await this.attendanceRepository.checkIn(eventId, userId);
    ...
    if (!attendance) {
      throw new NotFoundException('No se pudo encontrar la asistencia después del check-in');
    }
    
    return attendance;
  }

  private validateParams(params: CheckInParams): void {
    if (!params.eventId) {
